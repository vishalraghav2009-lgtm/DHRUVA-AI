...
import { Globe, ShieldAlert, Target, Zap, Info, TrendingUp, TrendingDown, Flag, MapPin } from 'lucide-react';
...
const Geopolitics: React.FC = () => {
  ...
  const filteredEvents = activeTab === 'all' 
    ? MOCK_GEO 
    : MOCK_GEO.filter(e => e.type.toLowerCase() === activeTab);
    ...
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-6 duration-700">
      ...
      <div className="grid grid-cols-1 gap-8">
        {filteredEvents.map((event, idx) => (
          <div key={idx} className="bg-gray-900 border border-gray-800 rounded-[2.5rem] overflow-hidden hover:border-blue-500/30 transition-all flex flex-col md:flex-row">
            <div className="md:w-1/3 p-10 bg-gradient-to-br from-gray-900 to-black border-r border-gray-800">
              ...
              <h3 className="text-2xl font-bold text-white mb-6 leading-tight">{event.event}</h3>
      ...
      <div className="flex items-center justify-center gap-2 text-gray-600 text-[10px] font-bold uppercase tracking-widest border-t border-gray-900 pt-8">
        <ShieldAlert size={14} /> All geopolitical insights are AI-generated interpretations of complex variables.
      </div>
    </div>
  );
};
